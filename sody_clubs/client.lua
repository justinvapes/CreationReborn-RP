...
AddEventHandler('sody_clubs:forceSync', function(xPlayer)
	...
	ESX.TriggerServerCallback('sody_clubs:getPlayerClub', function(playerdata)
		...
		if PlayerClub ~= nil and has_value(ClubList, PlayerClub) then
			zones = Config.Clubs[PlayerClub].Zones
			garage = Config.Clubs[PlayerClub].Garage
			clothes = Config.Clubs[PlayerClub].Clothes
			hasClub = true
			Citizen.Wait(500)
			displayClubMarkers(PlayerClub)
			playerZones()
...
function OpenChangingRoomMenu()
	local elements = {}
	table.insert(elements, {label = 'Clothes Locker', value = 'locker'})
	local ClubOutfits = Config.Clubs[PlayerClub].Clothes
	for i = 1, #ClubOutfits do
		table.insert(elements, {label = ClubOutfits[i]['label'], value = i})
	end
	
    ESX.UI.Menu.CloseAll()

    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'sody_cloakroom',
        {
        title    = 'Clothes',
	    align    = 'bottom-right',
		css      = 'apps',
        ...
        function(data, menu)
			...
			if data.current.value == 'locker' then
				menu.close()
				OpenChangingRoomMenu2()
			else
				TriggerEvent('skinchanger:getSkin', function(skin)		  
					if skin.sex == 0 then			  
						if ClubOutfits[value].male ~= nil then
							TriggerEvent('skinchanger:loadClothes', skin, ClubOutfits[value].male)
								Citizen.Wait(500)
								TriggerEvent('skinchanger:getSkin', function(skin)
						...
						if ClubOutfits[value].female ~= nil then
							TriggerEvent('skinchanger:loadClothes', skin, ClubOutfits[value].female)
								Citizen.Wait(500)
								TriggerEvent('skinchanger:getSkin', function(skin)
								TriggerServerEvent('esx_skin:save', skin)
							end)
...
function OpenChangingRoomMenu2()
    ESX.UI.Menu.CloseAll()
	ESX.TriggerServerCallback('esx_unicornjob:getPlayerDressing', function(dressing)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
			...
			TriggerEvent('skinchanger:getSkin', function(skin)
				ESX.TriggerServerCallback('esx_unicornjob:getPlayerOutfit', function(clothes)
