...
ESX.Game.GetPedRPNames = function()--New Global Function To Get Ped RP Names Client Side
	
	ESX.TriggerServerCallback('ESX:GetRPNames', function(cb)
        if cb then	          
	       Names = cb.firstname..' '..cb.lastname			   
        end
    end, data)
...
ESX.Game.UnregisterMugShot = function(mugshot, sound)--New Global Function To Unregister The Mugshots
...
ESX.Game.SpawnVehicleaa7b = function(modelName, coords, heading, cb)
...
ESX.Game.SpawnLocalVehicleaa7b = function(modelName, coords, heading, cb)
	local model = (type(modelName) == 'number' and modelName or GetHashKey(modelName))
...
ESX.Game.GetVehicleProperties = function(vehicle)
  ...
  return {
	...
	fuelLevel        = tostring(math.ceil(GetVehicleFuelLevel(vehicle))),
	...
	extraEnabled    = {
		...
		IsVehicleExtraTurnedOn(vehicle, 10),
		IsVehicleExtraTurnedOn(vehicle, 11),
		IsVehicleExtraTurnedOn(vehicle, 12)
	},
  }
end
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	...
	if props.fuelLevel ~= nil then
		...
		TriggerEvent("fuel:SetNewData", tonumber(props.fuelLevel))
	end
    ...
    if props.extraEnabled ~= nil then
        local extras = {}
        for k in pairs(props.extraEnabled) do
            if props.extraEnabled[k] == false then
                table.insert(extras, 1)
            else
                table.insert(extras, 0)
            end
        end
        SetVehicleExtra(vehicle, 1, extras[1])
        SetVehicleExtra(vehicle, 2, extras[2])
        SetVehicleExtra(vehicle, 3, extras[3])
        SetVehicleExtra(vehicle, 4, extras[4])
        SetVehicleExtra(vehicle, 5, extras[5])
        SetVehicleExtra(vehicle, 6, extras[6])
        SetVehicleExtra(vehicle, 7, extras[7])
        SetVehicleExtra(vehicle, 8, extras[8])
        SetVehicleExtra(vehicle, 9, extras[9])
        SetVehicleExtra(vehicle, 10, extras[10])
        SetVehicleExtra(vehicle, 11, extras[11])
        SetVehicleExtra(vehicle, 12, extras[12])
    end
