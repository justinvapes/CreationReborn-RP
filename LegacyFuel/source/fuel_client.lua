...
--Save Fuel To DB Every 2 Minutes
Citizen.CreateThread(function()
  while true do
	Citizen.Wait(120000)
    ...
    if IsPedInAnyVehicle(PlayerPedId(), false) then			  
	  ...
	  if driver == PlayerPedId() then	 
	     ...
	     TriggerServerEvent('eden_garage:savefuel', carplate, FuelValue)
	  end
    end
  end
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPedInAnyVehicle(PlayerPedId(), false) then			  
	  ...
	  if driver == PlayerPedId() then	 
        if GetVehicleFuelLevel(vehicle) <= 3 and GetIsVehicleEngineRunning(vehicle) then
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if currentCash - 3 >= currentCost then
...
RegisterNetEvent('fuel:SetNewData')
AddEventHandler('fuel:SetNewData', function(fuel)
						...
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if Config.UseESX then
								...
								if refillCost > 0 then
									if currentCash - 3 >= refillCost then
