...
Citizen.CreateThread(function()
    ...
    if Config.EnablePlayerManagement then
		if PlayerData.job and PlayerData.job.name == 'cardealer2' or PlayerData.job and PlayerData.job.name == 'offcardealer2' then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('33e0d0590-fab2-4c0f-92ee-dc842785023e3', function(hasEnoughMoney)
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('325bccb4c-cb37-4cf4-b9f2-8575d40332a3', function(hasEnoughMoney)
									if hasEnoughMoney then
										IsInShopMenu = false
										...
										ESX.Game.SpawnVehicleaa7b(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											if Config.EnableOwnedVehicles then
												if vehicleData.model == "thauler" or vehicleData.model == "enclosed" or vehicleData.model == "JetskiT" then	
													TriggerServerEvent('3dhyrjk-837hyr-oei84i-loe943', vehicleProps, 'trailer')
												else
													TriggerServerEvent('3dhyrjk-837hyr-oei84i-loe943', vehicleProps)
												end
											end
							...
							if hasEnoughMoney then
								...
								ESX.Game.SpawnVehicleaa7b(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
									...
									if Config.EnableOwnedVehicles then
										if vehicleData.model == "thauler" or vehicleData.model == "enclosed" or vehicleData.model == "JetskiT" then	
											TriggerServerEvent('3dhyrjk-837hyr-oei84i-loe943', vehicleProps, 'trailer')
											TriggerServerEvent('3dhyrjk-837hyr-oei84i-loe9435')
										else
											TriggerServerEvent('3dhyrjk-837hyr-oei84i-loe943', vehicleProps)
											TriggerServerEvent('3dhyrjk-837hyr-oei84i-loe9435')
										end
...
function OpenResellerMenu()
	...
	if PlayerData.job and PlayerData.job.name == 'offcardealer2' then
		table.insert(elements, {label = ('Go On Duty'), value = 'on_dutydealer'})
	end

	if PlayerData.job and PlayerData.job.name == 'cardealer2' then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elseif data.current.value == 'on_dutydealer' then
			...
			ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount) 
				...
				TriggerServerEvent('duty:cardealer2on')
				Citizen.Wait(1000)
			...
			TriggerServerEvent('BenefactorDutyLog', 'On')
	    ...
	    elseif data.current.value == 'off_dutydealer' then
			...
			TriggerServerEvent('BenefactorDutyLog', 'Off')
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('qi2yr-473kd-ldk3d-jf73s-3ls3s', GetPlayerServerId(closestPlayer), 'society_cardealer2', _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function (data2, menu2)
				menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				TriggerServerEvent('328ecc9e3-09e7-41db-aa833', model)
				...
				if Config.EnableOwnedVehicles then
					if CurrentVehicleData.model == "thauler" or CurrentVehicleData.model == "enclosed" or CurrentVehicleData.model == "JetskiT" then	
						TriggerServerEvent('31c72c501-8949-41f3-84a7-9e22c51e4a33', GetPlayerServerId(closestPlayer), vehicleProps, 'trailer')
					else	
						TriggerServerEvent('31c72c501-8949-41f3-84a7-9e22c51e4a33', GetPlayerServerId(closestPlayer), vehicleProps)
					end
					--TriggerServerEvent('31c72c501-8949-41f3-84a7-9e22c51e4a33', GetPlayerServerId(closestPlayer), vehicleProps)
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('c3733e85-777a-8285-4836-20b26c07edbc', 'cardealer2', function(data2, menu2)
				menu2.close()
			end)
...
AddEventHandler('esx_vehicleshop2:hasEnteredMarker', function (zone)
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and PlayerData.job and PlayerData.job.name == 'cardealer2' and PlayerData.job and PlayerData.job.grade_name == 'boss' then
