...
GlobalFunction = function(event, data)
    ...
    TriggerServerEvent("motel2:globalEvent", options)
end

RegisterNetEvent('Motel2:CheckPlayer')--Call this a few seconds after your character has loaded in. This will refesh the instance incase you logged out inside your or a players room
AddEventHandler('Motel2:CheckPlayer', function()

    ESX.TriggerServerCallback("motel2:fetchMotels", function(fetchedMotels, fetchedName)
        ...
        if fetchedName then
         ...
         TriggerEvent('Motel2:CheckPlayerS2') 
    end)
end)
...
OpenInviteMenu = function(motelRoomData)
    ...
    ESX.TriggerServerCallback("motel2:retreivePlayers", function(playersRetreived)
...
EnterMotel = function(motelRoomData)
	...
	if motelRoomData["room"] ==  2 or motelRoomData["room"] == 3 or motelRoomData["room"] == 4  or motelRoomData["room"] == 5 or motelRoomData["room"] == 6 or motelRoomData["room"] == 8 or motelRoomData["room"] == 9 or motelRoomData["room"] == 10 or motelRoomData["room"] == 11 then
	   SetEntityHeading(PlayerPedId(), 244.15)	
    elseif motelRoomData["room"] == 1 or motelRoomData["room"] == 7 then		
       SetEntityHeading(PlayerPedId(), 339.79)	   			   	   
	end	 	
       DoScreenFadeIn(1000)     
       FreezeEntityPosition(PlayerPedId(), false) 		
       ExitInstance()
...
OpenLandLord = function()
 ...
 Inmenu = true
    ...
    ESX.TriggerServerCallback('motel2:DoesOwn', function(Owned)
    if Owned then
	   SellMenu()
	else
    ...
    cachedData["cams"] = {
        ["first"] = CreateAnimatedCam({ ["x"] = 575.96, ["y"] = -1758.84,  ["z"] = 40.50, ["rotationX"] = -25.0, ["rotationY"] = 0.0, ["rotationZ"] = 100.0 }),     
    }
    ...
    Citizen.CreateThread(function()
        ...
        while cachedData["purchasing"] do
            ...
            BeginTextCommandDisplayHelp("Instructions2")
   ...
   local ownedMotel = GetPlayerMotel()
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)		
            ...
            TriggerServerEvent('motel2:SellMotel', ownedMotel)
            ESX.UI.Menu.CloseAll()
...
OpenConfirmBox = function(motelRoom)
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "main_accept_motel", {
        ...
        if action == "yes" then
            ...
            ESX.TriggerServerCallback("motel2:buyMotel", function(bought)
                if bought then                  
                   Cleanup()           
                   cachedData["purchasing"] = false
            ...
            controlDisables = {
               ...
               ESX.ShowNotification("~b~Paid ~g~$50,000 ~w~To LandLord.. You Now ~g~Own ~w~Room ~b~#" .. motelRoom)	
		       ClearPedTasks(PlayerPedId())
			   TriggerEvent("motel2:RefreshInit")--Refresh data and cache it after buying
			   Inmenu = false
          end)	  
             else
...
OpenWardrobe = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		...
		if data.current.value == 'player_dressing' then
                ...
                ESX.TriggerServerCallback('motel2:getPlayerDressing', function(dressing)
				local elements = {}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
					...
					TriggerEvent('skinchanger:getSkin', function(skin)
						ESX.TriggerServerCallback('motels2:getPlayerOutfit', function(clothes)
		...
		elseif data.current.value == 'remove_cloth' then
			...
			ESX.TriggerServerCallback('motel2:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					...
					menu2.close()
					TriggerServerEvent('motels2:removeOutfit', data2.current.value)
...
DoMotelAction = function(action, motelRoomData)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name ~= 'police' then
        ...
        ESX.TriggerServerCallback("lsrp-motels2:getPropertyInventory", function(cb)
            TriggerEvent('esx_inventoryhud:openMotelsInventory2', cb)
			Motel2DrawInventory = cb.weapons	
        end,ESX.GetPlayerData().identifier)	

        Citizen.Wait(700)	 
	    Total = #Motel2DrawInventory 
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name ~= 'police' then
        ...
        ESX.TriggerServerCallback("lsrp-motels2:getPropertyInventoryBed", function(cb)
            TriggerEvent('esx_inventoryhud:openMotelsInventoryBed2', cb)
			Motel2BedInventory = cb.weapons	
        end,ESX.GetPlayerData().identifier)
		
		Citizen.Wait(700)	 
	    Total = #Motel2BedInventory 
	    TriggerEvent('esx_inventoryhud:OpenTotal', Total)
	else
		exports['mythic_notify']:DoHudText('error', 'You Can Not Access This While On Duty')
	 end
...
RegisterNetEvent('Motel2:CheckPlayerS2')
AddEventHandler('Motel2:CheckPlayerS2', function()

    if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 346.15, -1000.96, -99.20, false) <= 7 or
       GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 337.81, -993.05, -99.20, false) <= 7  or
	   GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 345.12, -993.29, -99.20, false) <= 7  or
       GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 352.42, -993.98, -99.20, false) <= 7  or
       GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 350.37, -1007.35, -99.20, false) <= 7 then	   
...
Citizen.CreateThread(function()
    ...
    while not HasModelLoaded(GetHashKey("ig_abigail")) do
      ...
      local DoorPed =  CreatePed(4, 0x400AEC41, 569.77, -1746.45, 28.21, 247.28, false, true)
      SetEntityHeading(DoorPed, 247.28)
