...
GlobalFunction = function(event, data)
    ...
    TriggerServerEvent("motel3:globalEvent", options)
end

RegisterNetEvent('Motel3:CheckPlayer')--Call this a few seconds after your character has loaded in. This will refesh the instance incase you logged out inside your or a players room
AddEventHandler('Motel3:CheckPlayer', function()

    ESX.TriggerServerCallback("motel3:fetchMotels", function(fetchedMotels, fetchedName)
        ...
        if fetchedName then
         ...
         TriggerEvent('Motel3:CheckPlayerS2') 
    end)
end)
...
OpenMotelRoomMenu = function(motelRoom)
    ...
    if #menuElements == 0 then
        table.insert(menuElements, {
            ["label"] = "This Room Is Free To Rent"
        })
    end
...
OpenInviteMenu = function(motelRoomData)
    ...
    ESX.TriggerServerCallback("motel3:retreivePlayers", function(playersRetreived)
...
EnterMotel = function(motelRoomData)
	...
	if motelRoomData["room"] ==  1 or motelRoomData["room"] == 2 or motelRoomData["room"] == 3  then
	   SetEntityHeading(PlayerPedId(), 89.57)	
    elseif motelRoomData["room"] == 4 or motelRoomData["room"] == 5 or motelRoomData["room"] == 6 or motelRoomData["room"] == 7 or motelRoomData["room"] == 8 or motelRoomData["room"] == 9 or motelRoomData["room"] == 10 then		
       SetEntityHeading(PlayerPedId(), 8.61)	
     elseif motelRoomData["room"] == 11 or motelRoomData["room"] == 12 then		
       SetEntityHeading(PlayerPedId(), 270.37)	   
	end	 	
       DoScreenFadeIn(1000)     
       FreezeEntityPosition(PlayerPedId(), false) 		
       ExitInstance()
...
OpenLandLord = function()
 ...
 Inmenu = true
    ...
    ESX.TriggerServerCallback('motel3:DoesOwn', function(Owned)
    if Owned then
	   SellMenu()
	else
    ...
    cachedData["cams"] = {
        ["first"] = CreateAnimatedCam({ ["x"] = 1122.77, ["y"] = 2666.67,  ["z"] = 46.68, ["rotationX"] = -25.0, ["rotationY"] = 0.0, ["rotationZ"] = 900.0 }),     
    }
    ...
    Citizen.CreateThread(function()
        ...
        while cachedData["purchasing"] do
            ...
            BeginTextCommandDisplayHelp("Instructions3")
   ...
   local ownedMotel = GetPlayerMotel()
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)		
            ...
            TriggerServerEvent('motel3:SellMotel', ownedMotel)
            ESX.UI.Menu.CloseAll()
...
OpenConfirmBox = function(motelRoom)
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "main_accept_motel", {
        ...
        if action == "yes" then
            ...
            ESX.TriggerServerCallback("motel3:buyMotel", function(bought)
                if bought then                  
                    Cleanup()           
                    cachedData["purchasing"] = false
            ...
            controlDisables = {
               ...
               ESX.ShowNotification("~b~Paid ~g~$10,000 ~w~To LandLord.. You Now ~g~Own ~w~Room ~b~#" .. motelRoom)	
		       ClearPedTasks(PlayerPedId())
			   TriggerEvent("motel3:RefreshInit")--Refresh data and cache it after buying
			   Inmenu = false
          end)	  
             else
...
OpenWardrobe = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		...
		if data.current.value == 'player_dressing' then
                ...
                ESX.TriggerServerCallback('motel3:getPlayerDressing', function(dressing)
				local elements = {}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
					...
					TriggerEvent('skinchanger:getSkin', function(skin)
						ESX.TriggerServerCallback('motels3:getPlayerOutfit', function(clothes)
		...
		elseif data.current.value == 'remove_cloth' then
			...
			ESX.TriggerServerCallback('motel3:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					...
					menu2.close()
					TriggerServerEvent('motels3:removeOutfit', data2.current.value)
...
DoMotelAction = function(action, motelRoomData)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name ~= 'police' then
        ...
        ESX.TriggerServerCallback("lsrp-motels3:getPropertyInventory", function(cb)
            TriggerEvent('esx_inventoryhud:openMotelsInventory3', cb)
			Motel3DrawInventory = cb.weapons
        end,ESX.GetPlayerData().identifier)	

        Citizen.Wait(700)	 
	    Total = #Motel3DrawInventory 
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name ~= 'police' then
        ...
        ESX.TriggerServerCallback("lsrp-motels3:getPropertyInventoryBed", function(cb)
            TriggerEvent('esx_inventoryhud:openMotelsInventoryBed3', cb)
			Motel3BedInventory = cb.weapons	
        end,ESX.GetPlayerData().identifier)
		
		Citizen.Wait(700)	 
	    Total = #Motel3BedInventory 
	    TriggerEvent('esx_inventoryhud:OpenTotal', Total)
	else
		exports['mythic_notify']:DoHudText('error', 'You Can Not Access This While On Duty')
	end
...
RegisterNetEvent('Motel3:CheckPlayerS2')
AddEventHandler('Motel3:CheckPlayerS2', function()

    if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 265.76, -1003.05, -99.01, false) <= 7 or
       GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 262.13, -996.71, -99.01, false) <= 7 or
	   GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 260.70, -1003.31, -99.01, false) <= 7 or
       GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 255.54, -1000.76, -99.01, false) <= 7 then	   
...
Citizen.CreateThread(function()
    ...
    while not HasModelLoaded(GetHashKey("ig_abigail")) do
      ...
      local DoorPed2 =  CreatePed(4, 0x400AEC41, 1142.39, 2664.03, 37.17, 97.38, false, true)
      SetEntityHeading(DoorPed2, 97.38)
      FreezeEntityPosition(DoorPed2, true)
      SetEntityInvincible(DoorPed2, true)
      SetBlockingOfNonTemporaryEvents(DoorPed2, true)
	  SetModelAsNoLongerNeeded(DoorPed2)
end)