...
RegisterNetEvent('mdt:sendTicket')
AddEventHandler('mdt:sendTicket', function(data, offenderID)
...
RegisterNetEvent('mdt:sendToComServ')
AddEventHandler('mdt:sendToComServ', function (offenderID, data)
  local offender = offenderID
  local comActions = data

  TriggerServerEvent("esx_communityservice:sendToCommunityService", offender, comActions)
end)

RegisterNetEvent('mdt:sendToJail')
AddEventHandler('mdt:sendToJail', function (offenderID, data, reason)
  local offender = offenderID
  local jailTime = data
  local jailReason = reason

  TriggerServerEvent("hf736-3ihr6-49d04-lkfh34", offender, jailTime, jailReason)
end)
...
function DeleteNetObj(obj)
    ...
    TriggerServerEvent('mdt:DeleteNetObj', tablet)
end

RegisterNetEvent("mdt:DeleteNetObj")
AddEventHandler("mdt:DeleteNetObj", function(obj)
