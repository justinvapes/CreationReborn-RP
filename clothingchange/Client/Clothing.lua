...
function PoliceToggleProps(which, extra)
	if Cooldown then return end
	local Prop = Props[which]
	local Ped = PlayerPedId()
	local Gender = IsMpPed(Ped)
...
function PoliceToggleClothing(which)
	if Cooldown then return end
	local Toggle = Drawables[which] if extra then Toggle = Extras[which] end
	local Ped = PlayerPedId()
	local Cur = { -- Lets check what we are currently wearing.
...
RegisterNetEvent('clothingchange:policeremove')
AddEventHandler('clothingchange:policeremove', function()
	PoliceToggleProps("Hat") 
	Citizen.Wait(2000)
	PoliceToggleClothing("Mask") 
end)
