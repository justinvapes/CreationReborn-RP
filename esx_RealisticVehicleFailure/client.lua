		...
		if isPedDrivingAVehicle() then
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					local armor = GetVehicleMod(vehicle, 16)

					if healthEngineCombinedDelta == healthBodyDeltaScaled or healthEngineCombinedDelta == healthPetrolTankDeltaScaled then
						-- If huge damage trunk then increase damage
						if healthEngineCombinedDelta > (healthEngineCurrent - cfg.engineSafeGuard) then
							if armor == 4 then
								healthEngineCombinedDelta = healthEngineCombinedDelta * 9
							elseif armor == 3 then
								healthEngineCombinedDelta = healthEngineCombinedDelta * 11
							elseif armor == 2 then
								healthEngineCombinedDelta = healthEngineCombinedDelta * 13
							elseif armor == 1 then
								healthEngineCombinedDelta = healthEngineCombinedDelta * 14
							else
								healthEngineCombinedDelta = healthEngineCombinedDelta * 15
							end
							-- print('Scaled: ', healthEngineCombinedDelta)
						end
					
						healthEngineCombinedDelta = healthEngineCombinedDelta * 0.03

					else
						healthEngineCombinedDelta = healthEngineCombinedDelta * cfg.EngineDamageMultiplier						
						if armor == 4 then
							healthEngineCombinedDelta = healthEngineCombinedDelta * 0.7
						elseif armor == 3 then
							healthEngineCombinedDelta = healthEngineCombinedDelta * 0.8
						elseif armor == 2 then
							healthEngineCombinedDelta = healthEngineCombinedDelta * 0.85
						elseif armor == 1 then
							healthEngineCombinedDelta = healthEngineCombinedDelta * 0.90
						end
					end
			...
			if healthEngineNew ~= healthEngineCurrent then
				if GetVehicleEngineHealth(vehicle) > 100 then
