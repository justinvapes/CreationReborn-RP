...
RegisterNUICallback('register', function(data, cb)
	...
	for theData, value in pairs(myIdentity) do
		...
		elseif theData == "height" then
			...
			if height then
				if height > 200 or height < 50 then
	...
	ESX.TriggerServerCallback('esx_identity:IsNameTaken', function (IsNameTaken) 
	if not IsNameTaken then
		print(data.sex)
		TriggerServerEvent('esx_identity:setIdentity', data, myIdentifiers)
		EnableGui(false)
		Citizen.Wait(500)	
		TriggerEvent('showSkinCreator', data.sex)	
		-- TriggerServerEvent('crumblecad:registerCitizen')			
        Citizen.Wait(5) 
        TriggerEvent('kashactersS:SetInvis')
        Citizen.Wait(10000)		       		
        TriggerServerEvent("motel:RefreshNames") --Motels1
        TriggerServerEvent("motel2:RefreshNames")--Motels2
        TriggerServerEvent("motel3:RefreshNames")--Motels3
        Citizen.Wait(1000)
...
function verifyName(name)
	...
	if nameLength > 31 or nameLength < 3 then
	SendNUIMessage({
       type = "namesize",
       size = true
    })
