...
local Components = {
	...
	{label = _U('pants_1'),					name = 'pants_1',			value = 0,		min = 0,	zoomOffset = -1.2,		camOffset = -0.5},
	{label = _U('pants_2'),					name = 'pants_2',			value = 0,		min = 0,	zoomOffset = -1.2,		camOffset = -0.5,	textureof	= 'pants_1'},
	...
	{label = _U('chain_1'),					name = 'chain_1',			value = 0,		min = 0,	zoomOffset = -1.8,		camOffset = 0.65},
	{label = _U('chain_2'),					name = 'chain_2',			value = 0,		min = 0,	zoomOffset = -1.8,		camOffset = 0.65,	textureof	= 'chain_1'},
...
function LoadDefaultModel(malePed, cb)
	...
	if LoadSkin['model'] ~= nil then 
		characterModel = LoadSkin['model']
	else
		
		ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(_cb)
			Citizen.Wait(250)
			if _cb ~= nil then --coz lua is retarded.
				if _cb[''] ~= nil then
					characterModel = _cb['model']
				else
					if malePed then
...
function ApplySkin(skin, clothes)
	...
	if Character['dad'] ~= nil and Character['mum'] ~= nil and Character['percent'] ~= nil then
		if Character['sex'] == 0 then
			SetPedHeadBlendData		(playerPed, Character['dad'], Character['mum'], Character['dad'], Character['skin'], Character['skin'], Character['skin'], Character['percent'] * 0.1, Character['percent'] * 0.1, 0.0, true)
		elseif Character['sex'] == 1 then
			SetPedHeadBlendData		(playerPed, Character['dad'], Character['mum'], Character['mum'], Character['skin'], Character['skin'], Character['skin'], Character['percent'] * 0.1, Character['percent'] * 0.1, 0.0, true)
		end
	else
