(function(){
	...
	let MenuTpl =
		'<div id="menu_{{_namespace}}_{{_name}}" class="dialog {{#isBig}}big{{/isBig}}">' +
			'<div class="headers"><div class="head"></div>' +
				'<div class="categorie"><span>{{title}}</span></div></div>' +
				'<div class="footers">' +
				'<center>{{#isDefault}}<input type="text" name="value" id="inputText"/>{{/isDefault}}</center>' +
				'{{#isBig}}<textarea name="value"/>{{/isBig}}' +
				'<div class="buttons"><button type="button" name="submit">Ok</button>' +
				'<button type="button" name="cancel">Cancel</button></div>' +
				'</div>'
			'</div>' +
		'</div>'
	;
