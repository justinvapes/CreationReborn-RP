...
RegisterNetEvent('esx_lscustomBennys:installMod')
AddEventHandler('esx_lscustomBennys:installMod', function()
	local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_lscustomBennys:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_lscustomBennys:cancelInstallMod')
AddEventHandler('esx_lscustomBennys:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
...
function OpenVehicleExtrasMenu()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_extras', {
		...
		TriggerServerEvent('BennysPartsLog', mod, plate)
		TriggerServerEvent("esx_lscustomBennys:buyMod", 0)
...
function OpenLSMenu(elems, menuname, menutitle, parent)
	...
	ESX.UI.Menu.Open(
					...
					if k == data.current.modType or isRimMod then
						if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
							...
							if isRimMod then
								...
								TriggerServerEvent('BennysPartsLog', mod, plate)
								TriggerServerEvent("esx_lscustomBennys:buyMod", data.current.price)
							else
								mod = data.current.modType
								if mod == nil then
									mod = 'Unknown'
								...
								TriggerServerEvent('BennysPartsLog', mod, plate)
								TriggerServerEvent("esx_lscustomBennys:buyMod", v.price)
							end
						end
						menu.close()
						found = true
		...
		function(data, menu)
			menu.close()
			TriggerEvent('esx_lscustomBennys:cancelInstallMod')
