...
Citizen.CreateThread(function()
    while true do
        local sleep = 1000
        if ESX ~= nil and ESX.GetPlayerData().job and ESX.GetPlayerData().job.name == 'nightclub' then
            for a = 1, #SecurityCamConfig.Locations do
                ...
                if distance <= 0.5 then
                    ...
                    DrawM('[~b~Press ~w~[~g~E~w~] To Check The ~b~Cameras~w~]', 27, box_x, box_y, box_z)	
...
function DrawM(hint, type, x, y, z)
	ESX.Game.Utils.DrawText3D({x = x, y = y, z = z}, hint, 0.3)
end