...
RegisterKeyMapping('phone', 'Toggle Phone', 'keyboard', 'grave')
...
RegisterNUICallback('sendMessage', function(data, cb)
  if data.message ~= 'nil' then
    if data.message ~= nil then
      ...
      exports['mythic_notify']:DoHudText('error', 'Message was empty')
    end
  else
		exports['mythic_notify']:DoHudText('error', 'Message was empty')
  end
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  if hasFocus == true then
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
      local url = data.url
			-- exports['screenshot-basic']:requestScreenshotUpload(data.url, data.field, {encoding = 'jpg', quality = 0.50}, function(data)
      --   local resp = json.decode(data)
      --   DestroyMobilePhone()
      --   CellCamActivate(false, false)
      --   resp.url = url .. '/' .. resp.filename
      --   --cb(json.encode({ url = resp.files[1].url }))   
      --   cb(json.encode({ url = resp.url }))
      -- end)
      exports['screenshot-basic']:requestScreenshotUpload("https://discord.com/api/webhooks/851750893362741276/mF-QHL73KIJljyUcZqJgnuBjK12dJhPHvAVvq3oKEgimmTrU6C_bjySpow_cow0hMffk", data.field, function(data)
