...
function startAnimLoop(lib, anim)
...
function OpenAnimationsSubMenu(menu)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			elseif type == 'Stopit' then				
				ClearPedTasks(GetPlayerPed(-1))
				
			elseif type == 'loop' then				
				startAnimLoop(lib, anim)
...
AddEventHandler('Animations:OpenMenu', function()
 if not IsPedSittingInAnyVehicle(PlayerPedId()) then
	    ...
	    exports['mythic_notify']:DoHudText('error', 'You Can Not Use This While In A Vehicle')	
	end
 end)
