...
function OpenMecanoActionsMenu()
  ...
  local elements = {
    ...
    if PlayerData.job and PlayerData.job.name == 'mecano2' then
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
            ...
            local elements = {
              {label = _U('flat_bed'),  value = 'f100tow'},
            }
              ...
              function(data, menu)
                ...
                for i=1, #elements, 1 do
                  if Config.MaxInService == -1 then
                    ...
                    ESX.Game.SpawnVehicleaa7b(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 69.8, function(vehicle)
    ...
    if health >= 180 then
		...
		TriggerServerEvent('LSCustomsDutyLog', 'Off')
		ESX.UI.Menu.CloseAll()
		Citizen.Wait(250)
		OpenMecanoActionsMenu()
        end)
    ...
    if health >= 180 then
	  ...
	  if male or femalemale then	
        ...
        setUniformHellRiders(data.current.value, playerPed)
		TriggerServerEvent('duty:mecanoHSetOn')
		TriggerServerEvent('LSCustomsDutyLog', 'On')
        ESX.UI.Menu.CloseAll()
		Citizen.Wait(250)
		OpenMecanoActionsMenu()		
	else
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('c3733e85-777a-8285-4836-20b26c07edbc', 'mecano2', function(data, menu)--esx_society:openBossMenu
          menu.close()
        end)
      end
...
function setUniformHellRiders(job, playerPed)
          ...
          function(data, menu)
            ...
            if amount == nil or amount < 0 then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('qi2yr-473kd-ldk3d-jf73s-3ls3s', GetPlayerServerId(closestPlayer), 'society_mecano2', _U('mechanic'), amount)
              end
            end
          end,
        function(data, menu)
      ...
      if data.current.value == 'del_vehicle' then
        ...
        if (DoesEntityExist(PlayerPedId()) and not IsEntityDead(PlayerPedId())) then
          ...
          if (IsPedSittingInAnyVehicle( PlayerPedId())) then
            ...
            local inFrontOfPlayer = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, distanceToCheck, 0.0)
...
AddEventHandler('esx_Mechanics:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_Mechanics:hasExitedMarker', function(zone)

  if zone == 'Garage' then
    TriggerServerEvent('esx_Mechanics:stopHarvest')
    TriggerServerEvent('esx_Mechanics:stopHarvest2')
    TriggerServerEvent('esx_Mechanics:stopHarvest3')
  end
...
AddEventHandler('esx_Mechanics:hasEnteredEntityZone', function(entity)

  local playerPed = GetPlayerPed(-1)

  if PlayerData.job and PlayerData.job.name == 'mecano2' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('esx_Mechanics:hasExitedEntityZone', function(entity)
...
Citizen.CreateThread(function()
    ...
    AddTextComponentString(_U('HardlineGarage'))
...
Citizen.CreateThread(function()
  while true do
    ...
    if PlayerData.job and PlayerData.job.name == 'mecano2' or PlayerData.job and PlayerData.job.name == 'offmecano2' then
    ...
    if PlayerData.job and PlayerData.job.name == 'mecano2' or PlayerData.job and PlayerData.job.name == 'offmecano2' then
       ...
       local currentZone = nil
        ...
        if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
           ...
           TriggerEvent('esx_Mechanics:hasEnteredMarker', currentZone)
        end
		
        if not isInMarker and HasAlreadyEnteredMarker then
           HasAlreadyEnteredMarker = false
           TriggerEvent('esx_Mechanics:hasExitedMarker', LastZone)
        end
	else
	    Citizen.Wait(250)
    end
  ...
  while true do
    ...
    if distance ~= -1 and distance <= 3.0 then
      ...
      if LastEntity ~= entity then
        TriggerEvent('esx_Mechanics:hasEnteredEntityZone', entity)
        LastEntity = entity
      end
      ...
      if LastEntity ~= nil then
        TriggerEvent('esx_Mechanics:hasExitedEntityZone', LastEntity)
        LastEntity = nil
      end
    end
  end
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'mecano2' then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
             ...
             TriggerServerEvent('esx_society:putVehicleInGarage', 'mecano2', vehicleProps)
         else
		 
            if GetEntityModel(vehicle) == GetHashKey('flatbed') or
			   GetEntityModel(vehicle) == GetHashKey('Reapers') or
			   GetEntityModel(vehicle) == GetHashKey('towtruck') or
               GetEntityModel(vehicle) == GetHashKey('towtruck2') or
               GetEntityModel(vehicle) == GetHashKey('slamvan3') then
               TriggerServerEvent('esx_service:disableService', 'mecano2')
            end
          end
        ...
        if IsControlJustReleased(0, Keys['F6']) and PlayerData.job and PlayerData.job.name == 'mecano2' then
 ...
 while true do
    ...
    if CurrentAction ~= nil then
       ...
       DisplayHelpTextFromStringLabel(0, 0, 1, -1)
        ...
        if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'offmecano2' then
         if CurrentAction == 'mecano_actions_menu' then
            OpenMecanoActionsMenu()
         end
            CurrentAction = nil
...
while true do
	...
	if PlayerData.job and PlayerData.job.name ~= 'mecano2' and PlayerData.job.name ~= 'police' then
	   if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
...
Citizen.CreateThread(function()
  while true do
	...
	local sleep = 1000		
	local coords = GetEntityCoords(PlayerPedId())
	local coords2 = vector3(-322.78, -146.34, 39.02)
	
   if PlayerData.job and PlayerData.job.name == 'mecano2' then
   
    if GetDistanceBetweenCoords(coords, coords2, true) < 5 then
	   sleep = 5
	   DrawMarker(1, -322.78, -146.34, 38.08, 0, 0, 0, 0, 0, 0, 0.9001, 0.9001, 0.5001, 255, 0, 0, 165, 0,0, 0,0)
...
function OpenShopMenu()
    ...
    ESX.UI.Menu.Open(
        ...
        elements = {
			...
			{label = 'Engine Rebuild Kit = $100,000', value = 'buy_rebuildkit'},
			{label = 'Engineer Certificate = $50,000', value = 'buy_engineer'},
            }
        },
        function(data, menu)
		
        if data.current.value == 'buy_oil' then
		   TriggerServerEvent('esx_Mechanics:BuyItem', 'oil')			
        elseif data.current.value == 'buy_filter' then
			TriggerServerEvent('esx_Mechanics:BuyItem', 'filter')				
		elseif data.current.value == 'buy_rebuildkit' then
			TriggerServerEvent('esx_Mechanics:BuyItem', 'rebuildkit')	
		elseif data.current.value == 'buy_engineer' then
			TriggerServerEvent('esx_Mechanics:BuyItem', 'engineer')		
		 end		 
       end,
       function(data, menu)
       menu.close()
