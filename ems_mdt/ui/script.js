    ...
    methods: {
        ...
        OffenderSearch() {
            if (this.offender_search) {
                ...
                $.post('http://ems_mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://ems_mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://ems_mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://ems_mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && this.report_new.incident) {
                $.post('http://ems_mdt/submitNewReport', JSON.stringify({
                    title: this.report_new.title,
                    char_id: this.report_new.char_id,
                    name: this.report_new.name,
                    fines: this.report_new.fines,
                    jail: this.report_new.jail,
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://ems_mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://ems_mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://ems_mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://ems_mdt/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://ems_mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://ems_mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://ems_mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://ems_mdt/getReport', JSON.stringify({
                id: id
            }));
            this.modal = 'loading';
            return;
        ...
        AttachToCall(index) {
            $.post('http://ems_mdt/attachToCall', JSON.stringify({
                index: index,
        ...
        DetachFromCall(index) {
            $.post('http://ems_mdt/detachFromCall', JSON.stringify({
                index: index
            }));
            this.current_call = {
                source: null,
        ...
        SetCallWaypoint(index) {
            $.post('http://ems_mdt/setCallWaypoint', JSON.stringify({
                coords: this.calls[index].coords
            }));
            return;
        },
        ...
        SaveEditCall() {
            $.post('http://ems_mdt/editCall', JSON.stringify({
        ...
        DeleteCall(index) {
            $.post('http://ems_mdt/deleteCall', JSON.stringify({
                index: index
            }));
            return;
        },
        ...
        SaveVehicleEditChanges() {
            $.post('http://ems_mdt/saveVehicleChanges', JSON.stringify({
        ...
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                if (mdtApp.current_call.id == mdtApp.calls[event.data.call].id) {
                    $.post('http://ems_mdt/deleteCallBlip')
                    mdtApp.current_call = {
                        source: null,
                        details: null,
                        id: null,
